...
//		a.setContent(" ");	// top
	...
	private XMLObject nextComplexElement(XMLObject element) {
		for (XMLObject child : element.getChildren()) {
			if (schemaElementNames.complexType.equals(child.getElementName())) {
				return child;
			} else if (schemaElementNames.sequence.equals(child.getElementName())) {
				...
				XMLObject obj = nextComplexElement(child);
				if (obj != null) {
					return obj; 
				}
			}
	...
	private void mapSubAttribute(Html html, XMLObject element) {
...
//			p.setClazz("name");
//			p.setContent(name);
			div.add(HEADER_MEDIUM).setContent(name + " <small>- element</small>");
		}
		...
		mapSubChildElement(html,element);
	}

	private void mapSubChildElement(Html html, XMLObject element) {
		// get down to next level of complexType or sequence
		XMLObject subChild = nextComplexElement(element);
		if (subChild != null) {
			mapComplexType(html, subChild);
		}
	}
