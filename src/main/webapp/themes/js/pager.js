...
function captureElementInputs(element)
{
	...
	var prefix = "";
	for (var x = 0; x < inputs.length; x++) {
		if (inputs[x].name && inputs[x].value && inputs[x].disabled == false && inputs[x].readOnly != true) {
			var capture = true;
...
function show(id) {
	try {
		...
		console.log("EX:show(" + id + ") - " + err.message);
	}
}
...
function updateRecord(id, inputParams, confirmMsg) {
