...
public class TestSaveRecordAction
{

	private final static Logger log = LoggerFactory.getLogger(TestSaveRecordAction.class);
	...
	public void testSaveName() throws Exception
	{
		...
		params.add(new HttpParam("person.firstname", "fred"));
		params.add(new HttpParam("person.lastname", "flinstone"));
		params.add(new HttpParam(ClientParamNames.TABLE_NAME_MAP_ENTRY, "person"));
		...
		SaveRecordAction saveRecord = new SaveRecordAction();
		try {
			String s = saveRecord.execute(execContext);
			if (s.startsWith("EX")) {
				fail(s);
			}
			log.debug("response from saveRecord [" + s + "]");
	...
	public void testSaveNameAndAddress() throws Exception
	{

		List<HttpParam> params = new ArrayList<HttpParam>();
		params.add(new HttpParam("person.firstname", "fred"));
		params.add(new HttpParam("person.lastname", "flinstone"));
		params.add(new HttpParam("address.street", "main street"));
		params.add(new HttpParam("address.city", "bedrock"));
		params.add(new HttpParam(ClientParamNames.TABLE_NAME_MAP_ENTRY, "person"));
		params.add(new HttpParam(ClientParamNames.STORAGE_CONFIG_REF, "storageConfig"));
		execContext.put(PagerWebConst.REQUEST_LIST, params);
